...
def read_ants_affine(mat_file):
    ...
    # ANTs stores transforms in ITK format - we need to convert
    with open(mat_file, 'r') as f:
        lines = f.readlines()
    
    # Parse ITK transform file
    params = None
    fixed_params = None
    for line in lines:
    ...
    # Parameters: r11 r12 r13 r21 r22 r23 r31 r32 r33 t1 t2 t3
    R = params[:9].reshape(3, 3)
    t = params[9:12]
    center = fixed_params[:3] if fixed_params is not None else np.zeros(3)
    
    # Build 4x4 transformation matrix
    # ITK transform: x' = R * (x - center) + center + t
    # Which simplifies to: x' = R*x + (center - R*center + t)
    T = np.eye(4)
    T[:3, :3] = R
    T[:3, 3] = center - R @ center + t
    
    # ANTs/ITK uses LPS coordinate system, FreeSurfer uses RAS
    # Apply coordinate conversion: flip X and Y
